...
Hull tried to update a revision that isn't in the storage backend — usually the
...
The plan file's `apiVersion` / `kind` aren't `hull/v1` / `Plan` — an older plan
...
A tar entry has an absolute path — the archive is hostile or malformed. Hull
refuses to extract.
