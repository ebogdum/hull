...
|---|---|---|
| **Layer** (`layers:` in `hull.yaml`) | Templates and values compose into **one** release. | The pieces are not separately useful — they always ship together. |
...
my-app/
├── hull.yaml                # has layers: [shared-base]
├── values.yaml
└── templates/
...
- `HULL_OCI_PLAIN_HTTP=true` — use HTTP instead of HTTPS (for local registries).
- `HULL_OCI_INSECURE_SKIP_TLS=true` — skip TLS verification (for self-signed registries).
...
For deterministic builds, layer order matters even when no field collides — the merge is documented stably so a `hull diff` between two installs is meaningful.
...
**Commit `hull.lock`** — without it, two builds of the same package can pull different versions of the same layer if the constraint allows it.
